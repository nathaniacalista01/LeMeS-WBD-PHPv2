<?php
    ...
    $course = new Course();
    $all_courses = $course->searchCourses($data);
    $joined_courses = $_SESSION["courses_enrolled"];
    
    $max_page = ceil(count($course->searchCourses($data))/4);
    $courses = $course->searchFewCourses($_GET["page"],$data);
    echo json_encode(array("courses" => $courses,"max_page" => $max_page,"joined_courses" => $joined_courses));
